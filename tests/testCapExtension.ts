...
class TestConsistentThaw extends Consistent{
    value
    constructor(){
        super()
        this.value = 5
...
class TestEventualFreeze extends Eventual{
    value
    constructor(){
        super()
        this.value  = 5
...
let simpleThaw = ()=>{
    ...
    return app.libs.thaw(con).then((ev)=>{
        ...
        let act : FarRef<SimpleThawAct> = app.spawnActor(SimpleThawAct)
        act.getEv(ev)
    })
}
scheduled.push(simpleThaw)

class RemThawAct extends CAPActor{
    getCon(con){
...
let remThaw = ()=>{
    let act : FarRef<RemThawAct> = app.spawnActor(RemThawAct)
    let con = new TestConsistentThaw();
    return new Promise((resolve)=>{
        (act.getCon(con) as any).then((ev)=>{
            ev.onCommit(()=>{
                resolve(log("Remote Thaw",ev.value,6) as any)
            })
        })
    })
}
...
let freeze = ()=>{
    ...
    let act : FarRef<FreezeAct> = app.spawnActor(FreezeAct)
    act.getCon(con)
    return new Promise((resolve)=>{
        setTimeout(()=>{
            con.value.then((v)=>{
                ...
                let v2 = ev.value == 5
                let ok = v1 && v2
                resolve(log("Freeze",ok,true) as any)
            })
        },2000)
    })
